...
(defparser verbatim-text ()
  ...
  (apply #'concatenate 'string (drop-trailing-blanks lines)))

(defparser verbatim-blank-line () (try eol) "\n")

(defparser verbatim-line ()
  indentation
  (-> (many1 (not-followed-by eol any-char)) text)
  (or eol (try eod))
...
(defparser blockquote-element ()
  (try (not-followed-by (p (count 3 #\Space) (none-of #\Space))))
  (-> element e)
  e) ;; Could make this more concise by having (-> x) equivalent to (-> x e) e

(defparser linkdef ()
  (-> (between #\[ #\] (chars-until #\])) name) #\Space
...
(defparser link-contents ()
  (-> (many1 (text-until (or (tagged-or (one-of "|]")) tagged-text))) r)
  r)
...
(defparser subdoc-contents (name)
  (incf (state :subdoc-level))
  (-> (many1 element) elements)
  eod
  (decf (state :subdoc-level))
  `(,name ,@elements))
...
(defparser indentation ()
  (try (count (- (state :current) (state :so-far)) #\Space))
  (setf (state :so-far) (state :current)))

(defparser indented (n p)
  (with-state ((current (+ current n)))
    (try (look-ahead (count n #\Space)))
    (-> p)))

(defparser extra-indentation (n)
  (setf (state :current) (+ (state :current) n))
  (setf (state :so-far) (+ (state :current) n)))
...
;; XXX: How do we tell whether the arguments to a combinator are the
;; names of parsers (as they both are here) or plain values (like n in
;; the indented combinator)?
(defparser not-followed-by (p1 p2)
  )
